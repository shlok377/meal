    ...
    <div className="absolute inset-0 w-full h-full min-h-screen flex flex-col justify-between pointer-events-none select-none overflow-hidden p-6 md:p-12 z-20">
      ...
      <header className="w-full flex items-center justify-between pointer-events-auto">
        ...
        <div className="flex items-center gap-4 md:gap-8">
          ...
          {/* Theme Toggle Pill (using opacity instead of blur) */}
          <button
      ...
      {/* 2. CENTER FLOATING CAMERA PRESET PILLS (using opacity instead of blur) */}
      <div className="w-full flex justify-center pointer-events-auto my-auto pt-64 md:pt-80">
        <div className="flex items-center gap-1.5 p-1 rounded-full border border-neutral-900/10 dark:border-neutral-100/10 bg-white/80 dark:bg-neutral-900/85 shadow-lg pointer-events-auto">
          {PRESET_OPTIONS.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-sans font-medium tracking-wider uppercase transition-all duration-300 cursor-pointer pointer-events-auto ${
                  isActive
      ...
      {/* 3. FOOTER / EDITORIAL DETAILS SECTION */}
      <footer className="w-full flex flex-col md:flex-row items-end md:items-end justify-between gap-6 pointer-events-auto pb-2">
        ...
        <div className="hidden lg:flex items-center gap-2 text-[11px] uppercase tracking-[0.25em] text-neutral-500/80 dark:text-neutral-400/70 pointer-events-none">
          <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span>
          <span>Drag to rotate 360°</span>
        </div>

        {/* Bottom Right: Narrative Copy & Order Button */}
        <div className="flex flex-col items-start md:items-end max-w-sm md:max-w-md text-left md:text-right space-y-4 pointer-events-auto">
