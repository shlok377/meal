...
export const EditorialOverlay: React.FC<EditorialOverlayProps> = ({
  ...
  return (
    <>
      {/* 1. LAYER 0 (z-0): REPEATING LUXURY STACKED DISPLAY TYPOGRAPHY (Behind 3D Canvas) */}
      <div 
        aria-hidden="true" 
        className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-0 select-none leading-[0.88] opacity-90 dark:opacity-85 overflow-hidden"
      >
      ...
      {/* 2. LAYER 2 (z-20): FOREGROUND EDITORIAL UI CONTROLS & DETAILS (In front of 3D Canvas) */}
      <div className="absolute inset-0 w-full h-full min-h-screen flex flex-col justify-between pointer-events-none select-none overflow-hidden p-6 md:p-12 z-20">
        ...
        <header className="w-full flex items-center justify-between pointer-events-auto">
          ...
          <div className="flex items-center gap-4 md:gap-8">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-neutral-800/20 dark:border-neutral-200/20 bg-white/50 dark:bg-neutral-900/60 backdrop-blur-md hover:bg-white/80 dark:hover:bg-neutral-800/90 transition-all duration-300 shadow-sm cursor-pointer"
            >
              {isDark ? (
                <>
        ...
        {/* Center Floating Camera Preset Pills */}
        <div className="w-full flex justify-center pointer-events-auto my-auto pt-60 md:pt-80">
          <div className="flex items-center gap-1.5 p-1 rounded-full border border-neutral-900/10 dark:border-neutral-100/10 bg-white/40 dark:bg-neutral-900/40 backdrop-blur-md shadow-lg">
        ...
        {/* Footer / Editorial Details Section */}
